...
def _model_fn(features, labels, mode, params):
    # Input Layer
    input_layer = tf.reshape(features[INPUT_TENSOR_NAME], [-1, 224, 224, 3])
    ...
    _logging.info("pool3: ", tf.shape(pool3))
    
    pool3_flat = tf.reshape(pool3, [-1, 64 * 8 * 2 * 49])
    
    _logging.info("pool3_flat: ", tf.shape(pool3_flat))
